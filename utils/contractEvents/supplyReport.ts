import { ethers } from 'ethers';
import contractArtifact from '../../artifacts/contracts/ShipChartering.sol/ShipTimeCharteringGeneric.json';  

export default async function supplyReport (contractAddress: string) {
  const contractABI: any[] = contractArtifact.abi;
 
  if (typeof window.ethereum !== 'undefined') {
    await window.ethereum.enable();
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    
    const signer = provider.getSigner();
    const charterContract = new ethers.Contract(contractAddress, contractABI, signer);

    const filter = charterContract.filters.SupplyReport();
    const events = await charterContract.queryFilter(filter);
    console.log("🚀 ~ file: supplyReport.ts:18 ~ events:", events)

    return 
  } else {
    console.error('Metamask is not installed or not connected.')
    return 
  }
}